...
def apply_hotel_ui():
    st.markdown(f"""
        ...
        .stChatMessage {{ border-radius: 20px; padding: 20px; margin-bottom: 15px; border: 1px solid rgba(255,255,255,0.1) !important; background-color: transparent !important; }}
        .stChatMessage[data-testid="stChatMessageAssistant"] {{ background-color: rgba(26, 75, 132, 0.2) !important; border-left: 4px solid {H_BLUE} !important; }}
        ...
        div[data-testid="stChatInput"] {{ border-radius: 15px !important; border: 1px solid {H_BLUE} !important; background-color: #0D2137 !important; }}
        .hotel-card {{ background: #0D2137; border: 1px solid rgba(26, 75, 132, 0.3); border-radius: 15px; overflow: hidden; margin-bottom: 20px; }}
...
def get_hotel_recommendations(query):
    ...
    q = query.lower()
    for p in db:
        if any(word in q for word in p['t'].lower().split()) or any(word in q for word in p['tp'].lower().split()):
            matches.append(p)
    return matches[:3]
